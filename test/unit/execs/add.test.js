...
describe("cmd: add", () => {
	...
	describe("With package.json", ()=>{
		...
		it("Fails: if package version collides with something already installed", ()=>{
			return TestHelper.installPackage(['roachy-stub@0.0.1']).then(()=>{
				FsHelper.changeCwd(TestHelper.getLibUiPath());
				return NpmExecHelper.install(["roachy-stub@0.0.2"]).then(()=>{
					...
					expect(pkgJson.dependencies).to.eql({"roachy-stub": "0.0.2"});

					let rootPkgJson = FsHelper.openPackageJson(TestHelper.formatPath());
					expect(rootPkgJson.dependencies).to.eql({"roachy-stub": "0.0.1"});
		...
		const types = [
			{
				descript: "deps",
				isDev: false,
				pkgDepType: "dependencies"
			},
			{
				descript: "devDeps",
				isDev: true,
				pkgDepType: "devDependencies"
			}
		];
		types.forEach(def => {
			it(`${def.descript}: adds app, adds packages to root, deletes node_modules`, ()=>{
				FsHelper.changeCwd(TestHelper.getLibUiPath());
				return NpmExecHelper.install(["roachy-stub@^0.0.2"], def.isDev).then(()=>{
					/**
					 * Ensure node_modules and package.json has what it needs
					 */
					expect(TestHelper.ensureFileExists([TestHelper.getLibUiPath(), "node_modules"])).to.equal(true);
					let pkgJson = FsHelper.openPackageJson(TestHelper.formatPath(TestHelper.getLibUiPath()));
					expect(pkgJson[def.pkgDepType]).to.eql({"roachy-stub": "0.0.2"});
					...
					return AddExec("lib-ui", TestHelper.getLibUiPath()).then(() => {
						...
						expect(rootConfig.getPackages()).to.have.property("roachy-stub");

						const appConfig = rootConfig.getApp("lib-ui");
						if(def.isDev) {
							expect(appConfig.getDevPackages()).to.eql(["roachy-stub"]);
						} else {
							expect(appConfig.getPackages()).to.eql(["roachy-stub"]);
						}
